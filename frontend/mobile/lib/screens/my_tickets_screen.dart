...
class _MyTicketsScreenState extends State<MyTicketsScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      final page = await _purchaseService.getMyTickets();
      final productIds = page.items.map((t) => t.productId).toSet();
      ...
      setState(() {
        _tickets = page.items;
        _productsById = {for (final p in products) if (p != null) p.id: p};
        _isLoading = false;
      });
    ...
    return Column(
      children: [
        Padding(
          ...
          child: Row(
            children: [
              _TabLabel(label: 'Nadolazeće', selected: _showUpcoming, onTap: () => setState(() => _showUpcoming = true)),
              const SizedBox(width: 20),
              _TabLabel(label: 'Iskorištene', selected: !_showUpcoming, onTap: () => setState(() => _showUpcoming = false)),
            ],
          ),
        ),
          ...
          child: _isLoading
              ...
              : _errorMessage != null
                  ...
                  : filtered.isEmpty
                      ? Center(
                          child: Text(
                            _showUpcoming ? 'Nemate nadolazećih ulaznica.' : 'Nemate iskorištenih ulaznica.',
                            ...
                            itemBuilder: (context, index) {
                              ...
                              return _TicketStubCard(
                                ...
                                onTap: () => Navigator.of(context).push(
                                  MaterialPageRoute(
                                    builder: (_) => TicketQrScreen(ticket: ticket, productName: product?.name ?? ticket.sectorName),
                                  ),
                                ),
                              );
                            },
  ...
  Widget build(BuildContext context) {
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(border: Border(bottom: BorderSide(color: selected ? primary : Colors.transparent, width: 2))),
        child: Text(
          label,
          style: TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: selected ? primary : tertiaryText),
        ),
      ),
    );
  }
...
class _TicketStubCard extends StatelessWidget {
  ...
  const _TicketStubCard({required this.ticket, required this.productName, required this.onTap});

  DateTime get _headlineDate => ticket.validDate ?? ticket.validFrom ?? ticket.createdAt;

  static const _months = ['JAN', 'FEB', 'MAR', 'APR', 'MAJ', 'JUN', 'JUL', 'AVG', 'SEP', 'OKT', 'NOV', 'DEC'];
            ...
            children: [
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    Text(_months[date.month - 1], style: const TextStyle(fontSize: 11, color: Colors.white70)),
                    Text('${date.day}', style: const TextStyle(fontSize: 22, fontWeight: FontWeight.w700, color: Colors.white)),
                  ],
                ),
              ),
              Expanded(
                child: Padding(
                  ...
                  child: Column(
                    ...
                    mainAxisSize: MainAxisSize.min,
                    children: [
                      Text(productName, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w700), maxLines: 1, overflow: TextOverflow.ellipsis),
                      const SizedBox(height: 4),
                      Text(
                        ticket.ticketTypeName != null ? '${ticket.sectorName} · ${ticket.ticketTypeName}' : ticket.sectorName,
