...
/// <c>delivery.RoutingKey</c> reads "notifications.email" — the queue name — not
/// the event's own key, so the dispatcher would reject a perfectly valid message as poison and
